...
function makeContext(options: {
  ...
  sessionFile?: string;
  selection?: string;
  onSelect?: (title: string, choices: string[]) => void;
}): ExtensionContext {
  const {
    cwd,
    hasUI = true,
...
describe("PolicyRegistry", () => {
  ...
  test("accepts policy registration through Pi's inter-extension event bus", async () => {
    ...
    let onRegistration: ((data: unknown) => void) | undefined;
    const registeredEvents = new Map<string, (event: unknown, context: unknown) => unknown>();
    const pi = {
      events: {
        on(channel: string, handler: (data: unknown) => void) {
          if (channel === POLICY_REGISTRATION_EVENT) onRegistration = handler;
          return () => undefined;
        },
      },
      on(event: string, handler: (data: unknown, context: unknown) => unknown) {
        registeredEvents.set(event, handler);
      },
    };

    permissionExtension(pi as unknown as ExtensionAPI);
    assert.ok(onRegistration);
    onRegistration({
      key: policyName,
      handler: ({ priority }: { priority: number }) => ({ decision: "deny", priority }),
    });
...
describe("Configuration parsing and resolution", () => {
  ...
  test("normalizes existing and future paths to absolute canonical paths", () => {
    ...
    try {
      ...
      assert.equal(
        normalizePath("alias/file.txt", temp),
        normalizePath("real/file.txt", temp),
      );
      assert.equal(
        normalizePath("alias/new/../new/file.txt", temp),
        join(realDirectory, "new", "file.txt"),
      );
  ...
  test("normalizes only path-valued arguments, not command strings", () => {
    const temp = makeTempDir();
    try {
      const parameters = normalizeToolParams(
        "bash",
        { command: "echo ./relative/path", timeout: 1000 },
        temp,
      );
  ...
  test("matches normalized paths, symlinks, interpolated variables, and regex suffixes", () => {
    ...
    try {
      ...
      assert.equal(
        matchRule(wildcardRule, "read", { path: join(temp, "outside.txt") }, cwd),
        true,
      );
  ...
  test("resolves priority first, then last matching rule, with a virtual ask fallback", () => {
    ...
    ] as unknown as Parameters<typeof resolveRules>[0];
...
describe("Tool-call permission middleware", () => {
  ...
  test("persists session, project, and global grants to their selected scopes", async () => {
    const cases = [
      { choice: "Allow only in this session", scope: "session" as const },
      { choice: "Allow always (Project)", scope: "project" as const },
      { choice: "Allow always (Global)", scope: "global" as const },
    ];
  ...
  test("keeps ephemeral session grants in memory only for this handler instance", async () => {
    ...
    try {
      ...
      assert.ok(logs.some(([, action, source]) =>
        action === "Allowed" && source === "session memory (ephemeral session)",
      ));
